...
public class TariffEyeQuery {
	...
	// #### CONSTANTS - Http header keys ######################################
    /** any header used by TariffEye should must start with this string **/
    private final static String HEADER = "TariffEye-";
	/** 
	 ...
	 */
	private final static String HEADER_SET_PARAMS = HEADER+"SetParameter";
	 ...
	 */
	private final static String HEADER_TAG_OK = HEADER+"OK";
	/** Subscription Expire TimeStamp **/
	private final static String HEADER_SUBSCR_EXPIRE = HEADER+"SubscrExpires";
	/** LastUpdate TimeStamp (from the server time) **/
	private final static String HEADER_LAST_UPDATE = HEADER+"LastUpdate";
	
	
	// #### FIELDS ############################################################
	/** Logger */
	private final static Logger m_log = Logger.getLogger(TariffEyeQuery.class);
	...
	public void addParam(String key,String value) {
	    assert connectStatus < 0 : 
	        "cannot modifiy paramters on a commited Query";
	    m_args.put(key,value);
	}
	
	// #### METHODS - access the web ##########################################
	/**
	 * Do the query.<BR>
	...
	// #### METHODS - private: Code for connection ############################
	...
	private void processHeaders() {
		assert m_connection != null : 
			"Must be connected before processing headers";
		...
		// check for subscription expire date
		if ((v = getHeaderValue(HEADER_SUBSCR_EXPIRE)) != null) {
		   handleSubscr(v);
		}
		
		// get the web server time
